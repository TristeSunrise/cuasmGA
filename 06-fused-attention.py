...
def call(q, k, v, M, o, grid, causal, sm_scale, kernel, load_dir):
    # shape constraints
...
def main():
    ...
    @triton_autotune_with_cache(
        ...
        key=['N_CTX'],
        ga_config=config,  # just need the path really
    )
    @triton.jit
    def tt_attn(
            Q, K, V, sm_scale, M, Out,  #
    ...
    ## TEST
    fgk_out = call(q, k, v, M, o, grid, causal, sm_scale, ga_kernel, load_dir)
    if config.tt:
    ...
    configs.append(
        triton.testing.Benchmark(
            ...
            ylabel="ms",
            plot_name=
            f"fused-attention-batch{BATCH}-head{N_HEADS}-d{D_HEAD}-fwd-causal={causal}",
            args={
                "H": N_HEADS,
                "BATCH": BATCH,
                "D_HEAD": D_HEAD,
    ...
    def bench_flash_attention(BATCH,
        ...
        if provider == "fgk":
            ...
            if config.load is None:
                ...
                load_dir = config.load
            fn = lambda: call(q, k, v, M, o, grid, causal, sm_scale, ga_kernel, load_dir)
            # if mode == "bwd":
            #     o = fn()
